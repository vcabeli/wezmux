...
impl Mux {
    ...
    pub fn notify(&self, notification: MuxNotification) {
        self.apply_notification_to_store(&notification);
        let mut subscribers = self.subscribers.write();
        subscribers.retain(|_, notify| notify(notification.clone()));
    }

    fn apply_notification_to_store(&self, notification: &MuxNotification) {
        match notification {
            MuxNotification::Alert {
                pane_id,
                alert:
                    wezterm_term::Alert::ToastNotification {
                        title,
                        body,
                        focus: _,
                    },
            } => {
                let workspace = self.workspace_for_pane(*pane_id);
                // Check if the pane is currently focused (client tracking or active pane)
                let focused_by_client = self
                    .clients
                    .read()
                    .values()
                    .any(|client| client.focused_pane_id == Some(*pane_id));
                let focused_by_active = self.is_pane_active(*pane_id);
                let unread = !focused_by_client && !focused_by_active;
                let title = sanitize_notification_text(title.as_deref().unwrap_or(body));
                ...
                match event.as_str() {
                    ...
                    "title" => {
                        if let Some(title) = data {
                            ...
                            if !title.is_empty() {
                                store.update_conversation_title(*pane_id, title);
                            }
                        }
                    }
                    "tool" => {
    ...
    fn workspace_for_pane(&self, pane_id: PaneId) -> String {
        self.resolve_pane_id(pane_id)
            .and_then(|(_, window_id, _)| self.get_window(window_id))
            .map(|window| window.get_workspace().to_string())
            .unwrap_or_else(|| self.active_workspace())
    }

    /// Check if a pane is the active pane of the active tab in the active workspace.
    fn is_pane_active(&self, pane_id: PaneId) -> bool {
            ...
            if let Some(window) = self.get_window(window_id) {
                if let Some(tab) = window.get_active() {
                    if let Some(active_pane) = tab.get_active_pane() {
                        if active_pane.pane_id() == pane_id {
                            return true;
                        }
                    }
                }
    ...
    pub fn mark_pane_notifications_read(&self, pane_id: PaneId) {
        self.notification_store.lock().mark_pane_read(pane_id);
    }
    ...
    pub fn most_recent_unread_notification_pane(&self) -> Option<PaneId> {
        self.notification_store.lock().most_recent_unread_pane()
    }

    pub fn latest_notification_for_workspace(
        &self,
        workspace: &str,
    ...
    pub fn consolidate_workspace_windows(&self, workspace: &str) -> Option<WindowId> {
        let window_ids = self.iter_windows_in_workspace(workspace);
        let primary_window_id = *window_ids.first()?;

        if window_ids.len() <= 1 {
            return Some(primary_window_id);
        }

        for window_id in window_ids.into_iter().skip(1) {
            let tabs = {
                let mut window = match self.get_window_mut(window_id) {
                    Some(window) => window,
                    None => continue,
                };
                let mut tabs = Vec::with_capacity(window.len());

                while !window.is_empty() {
                    let idx = window.len() - 1;
                    tabs.push(window.remove_by_idx(idx));
                }
            ...
            for tab in tabs {
                if let Err(err) = self.add_tab_to_window(&tab, primary_window_id) {
                    log::error!(
                        "while consolidating workspace `{workspace}` into window {primary_window_id}: {err:#}"
                    );
                }
            }
