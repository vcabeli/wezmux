    ...
    fn internal_parse(osc: &[&[u8]]) -> Result<Self> {
        ...
        match osc_code {
            ...
            SystemNotification => {
                ...
                // Handle OSC 9 with sub-parameters (e.g. \e]9;2;message\a from Claude Code).
                // ConEmu sub-codes 1,2,3,5 are ignored; the message body follows.
                // Also handles plain \e]9;message\a (2 elements).
                if osc.len() >= 3 {
                    // Sub-parameter style: join everything after the sub-code as the body
                    let body_parts: Vec<String> = osc[2..]
                        .iter()
                        .filter_map(|part| String::from_utf8(part.to_vec()).ok())
                        .collect();
                    let body = body_parts.join(";");
                    if body.is_empty() {
                        // Fall back to using osc[1] as the message
                        let s = String::from_utf8(osc[1].to_vec())?;
                        Ok(OperatingSystemCommand::SystemNotification(s))
                    } else {
                        Ok(OperatingSystemCommand::SystemNotification(body))
                    }
                } else {
