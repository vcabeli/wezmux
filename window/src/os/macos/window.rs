...
impl WindowView {
    ...
    extern "C" fn draw_rect(view: &mut Object, sel: Sel, _dirty_rect: NSRect) {
        // Wrap in catch_unwind to prevent panics from crossing the extern "C" FFI
        // boundary, which would cause panic_cannot_unwind → SIGABRT.
        let view_ptr = view as *mut Object;
        ...
        if let Err(err) = result {
            log::error!("draw_rect panicked: {:?}", err);
        }
    }
