...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let mut order: Vec<&'static str> = vec!["Wezmux", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    ...
                    } else if cmd.menubar[0] == "Wezmux" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("Wezmux {}", config::wezterm_version()),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        SpawnWindow => CommandDef {
            ...
            doc: "Launches the default program into a new tab in the current workspace, or a new window if no window is open".into(),
            keys: vec![(Modifiers::SUPER, "n".into())],
            args: &[],
            menubar: &["Shell"],
            icon: Some("cod_empty_window"),
        },
        PromptNewWorkspace => CommandDef {
            brief: "New Workspace".into(),
            doc: "Prompts for a workspace name and switches to it".into(),
            keys: vec![(Modifiers::SUPER.union(Modifiers::SHIFT), "n".into())],
            args: &[ArgType::ActiveWindow],
            menubar: &["Shell"],
        ...
        ToggleSidebar => CommandDef {
            ...
            doc: "Shows or hides the workspace sidebar".into(),
        ...
        QuitApplication => CommandDef {
            brief: "Quit Wezmux".into(),
            doc: "Quits Wezmux".into(),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://wezterm.org/" => CommandDef {
                ...
                doc: "Visit the Wezmux documentation website".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            ...
            "https://github.com/wezterm/wezterm/discussions/" => CommandDef {
                ...
                doc: "Visit Wezmux's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
            ...
            "https://github.com/wezterm/wezterm/issues/" => CommandDef {
                ...
                doc: "Visit Wezmux's GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
        ...
        ActivateWorkspaceByIndex(n) => {
            ...
            CommandDef {
                brief: format!("Activate {ordinal} workspace").into(),
                doc: format!(
                    "Switch to the {ordinal} workspace in sidebar order"
                )
                .into(),
                keys,
                args: &[ArgType::ActivePane],
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    return vec![
        ...
        ActivateWorkspaceByIndex(3),
        ActivateWorkspaceByIndex(4),
        ActivateWorkspaceByIndex(5),
        ActivateWorkspaceByIndex(6),
        ActivateWorkspaceByIndex(7),
        ActivateWorkspaceByIndex(8),
        ActivateWindow(0),
        ActivateWindow(1),
        ActivateWindow(2),
        ActivateWindow(3),
