...
impl super::TermWindow {
    ...
    fn leave_ui_item(&mut self, item: &UIItem) {
        match item.item_type {
            ...
            UIItemType::SidebarWorkspace(ref workspace) => {
                if self.sidebar.hovered_workspace.as_deref() == Some(workspace.as_str()) {
                    self.sidebar.hovered_workspace = None;
                }
            }
            UIItemType::SidebarCloseWorkspace(_)
            | UIItemType::SidebarNewWorkspace
            | UIItemType::SidebarNotificationBell
            | UIItemType::SidebarSplitHorizontal
    ...
    fn enter_ui_item(&mut self, item: &UIItem) {
        match item.item_type {
            UIItemType::TabBar(_) => {}
            UIItemType::SidebarWorkspace(ref workspace) => {
                self.sidebar.hovered_workspace.replace(workspace.clone());
            }
            UIItemType::SidebarCloseWorkspace(ref workspace) => {
                self.sidebar.hovered_workspace.replace(workspace.clone());
            }
            UIItemType::SidebarNewWorkspace
            | UIItemType::SidebarNotificationBell
        ...
        let ui_item = if matches!(self.current_mouse_capture, None | Some(MouseCapture::UI)) {
            ...
            match (self.last_ui_item.take(), &ui_item) {
                (Some(prior), Some(item)) => {
                    if prior != *item || !self.effective_use_fancy_tab_bar() {
    ...
    fn mouse_event_ui_item(
        ...
        // Handle scroll wheel on any sidebar UI item
        let is_sidebar_item = matches!(
            item.item_type,
            UIItemType::SidebarWorkspace(_)
                | UIItemType::SidebarCloseWorkspace(_)
                | UIItemType::SidebarNewWorkspace
                | UIItemType::SidebarNotificationBell
                | UIItemType::SidebarSplitHorizontal
        ...
        if is_sidebar_item {
            if let WMEK::VertWheel(amount) = event.kind {
                let scroll_delta = amount as f32 * 20.0;
                self.sidebar.scroll_offset = (self.sidebar.scroll_offset - scroll_delta).max(0.0);
                context.invalidate();
                return;
            }
        }
        ...
        match item.item_type {
            ...
            UIItemType::SidebarWorkspace(workspace) => {
                self.mouse_event_sidebar_workspace(workspace, event, context);
            }
            UIItemType::SidebarCloseWorkspace(workspace) => {
                self.mouse_event_sidebar_close_workspace(workspace, event, context);
            }
            UIItemType::SidebarNewWorkspace => {
                self.mouse_event_sidebar_new_workspace(event, context);
            }
            UIItemType::SidebarNotificationBell => {
                // TODO: open notification panel (Cmd+I)
    ...
    fn mouse_event_sidebar_workspace(
        &mut self,
        workspace: String,
        event: MouseEvent,
        context: &dyn WindowOps,
    ...
    fn mouse_event_sidebar_new_workspace(&mut self, event: MouseEvent, context: &dyn WindowOps) {
        if matches!(event.kind, WMEK::Press(MousePress::Left)) {
            self.show_new_workspace_prompt();
            context.invalidate();
        }
        context.set_cursor(Some(MouseCursor::Hand));
    }

    fn mouse_event_sidebar_close_workspace(
        &mut self,
        workspace: String,
        event: MouseEvent,
        context: &dyn WindowOps,
