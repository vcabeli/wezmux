...
impl super::TermWindow {
    pub fn spawn_command(&self, spawn: &SpawnCommand, spawn_where: SpawnWhere) {
        let spawn_where = match spawn_where {
            // Wezmux treats the workspace as the top-level container,
            // so creating another shell from inside it should stay in
            // the current workspace window as a new tab.
