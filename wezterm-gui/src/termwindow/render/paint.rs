...
impl crate::TermWindow {
    ...
    pub fn paint_pass(&mut self) -> anyhow::Result<()> {
        ...
        // Notification rings on panes with unread notifications
        self.paint_notification_rings(&panes, &mut layers)
            .context("paint_notification_rings")?;
    ...
    fn paint_notification_rings(
        &mut self,
        panes: &[mux::tab::PositionedPane],
        ...
        let top_bar_height = if self.show_tab_bar && !self.config.tab_bar_at_bottom {
            self.tab_bar_pixel_height().unwrap_or(0.) as f32
        } else {
            0.0
        };

        let ring_color = window::color::LinearRgba::with_srgba(0, 145, 255, 255);
        let ring_width = 2.5_f32;
