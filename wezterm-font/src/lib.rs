...
struct FontConfigInner {
    ...
    sidebar_meta_font: RefCell<Option<Rc<LoadedFont>>>,
...
impl FontConfigInner {
    ...
    pub fn new(config: Option<ConfigHandle>, dpi: usize) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            sidebar_meta_font: RefCell::new(None),
    ...
    fn config_changed(&self, config: &ConfigHandle) -> anyhow::Result<()> {
        ...
        *self.config.borrow_mut() = config.clone();
        ...
        self.sidebar_meta_font.borrow_mut().take();
    ...
    fn make_entity_font_impl(
        ...
        let make_bold = !matches!(
            entity,
            Entity::CommandPalette | Entity::SidebarBody | Entity::SidebarMeta
        );
        ...
        let (font_size, text_style) = match entity {
            ...
            // Sidebar body: regular weight, slightly smaller than title
            Entity::SidebarBody => {
    ...
    fn sidebar_body_font(&self, myself: &Rc<Self>) -> anyhow::Result<Rc<LoadedFont>> {
        let mut font = self.sidebar_body_font.borrow_mut();
        if let Some(entry) = font.as_ref() {
            return Ok(Rc::clone(entry));
        }
        let loaded = self.make_entity_font_impl(myself, Entity::SidebarBody)?;
        font.replace(Rc::clone(&loaded));
        Ok(loaded)
    }

    fn sidebar_meta_font(&self, myself: &Rc<Self>) -> anyhow::Result<Rc<LoadedFont>> {
        let mut font = self.sidebar_meta_font.borrow_mut();
        if let Some(entry) = font.as_ref() {
            return Ok(Rc::clone(entry));
        }
        let loaded = self.make_entity_font_impl(myself, Entity::SidebarMeta)?;
        font.replace(Rc::clone(&loaded));
        Ok(loaded)
    }
...
impl FontConfiguration {
    ...
    pub fn sidebar_meta_font(&self) -> anyhow::Result<Rc<LoadedFont>> {
        self.inner.sidebar_meta_font(&self.inner)
    }
